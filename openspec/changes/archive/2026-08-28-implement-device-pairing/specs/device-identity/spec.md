...
The client SHALL treat refusal of both session refresh and device-root recovery as device revocation, clear all local credentials and capability authorization, and expose a re-pairing-required state without deleting unrelated local user data.
...
- **THEN** the client deletes the secure credential record, disables authenticated work, clears usable capabilities, and exposes re-pairing required

#### Scenario: Explicit local sign out
- **WHEN** the user signs out this installation locally
- **THEN** the client deletes its secure credential record and clears session-scoped capability state without claiming that Platform revoked the device
