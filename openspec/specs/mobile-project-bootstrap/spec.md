...
The repository SHALL provide an Android application shell that compiles into a debug application and hosts the shared Ratatoskr Compose content without implementing feature UI.

#### Scenario: Android debug build
- **WHEN** the documented Android build command runs in a clean checkout with the pinned toolchain
- **THEN** it exits successfully and produces a Ratatoskr debug application artifact

### Requirement: iOS application shell builds shared content

The repository SHALL provide an iOS application shell that builds for the iOS Simulator, links the shared framework, and hosts the shared Ratatoskr Compose content without implementing feature UI.
...
- **THEN** it exits successfully and produces a Ratatoskr simulator application artifact linked to the shared framework

### Requirement: Shared module remains usable by both platform shells

The shared module SHALL expose the common Compose entry point and generated public Platform contract types to Android and iOS, and its common tests SHALL execute on supported platform test targets.
...
- **THEN** both consumers resolve the same shared Compose entry point without a platform-specific duplicate
...
The product CI SHALL lint the Kotlin and Swift sources, verify generated contract drift including the mutation test, run shared-module tests, build the Android application, build the unsigned iOS Simulator application, and validate OpenSpec artifacts.

#### Scenario: Clean change passes CI
- **WHEN** product CI runs on the committed bootstrap tree
- **THEN** all lint, contract, shared-test, Android-build, iOS-build, and OpenSpec jobs complete successfully

#### Scenario: Shared tests cannot be omitted
- **WHEN** the CI workflow definition is checked against the documented product gate
- **THEN** the check fails if the shared-module test command is absent from either source of truth
